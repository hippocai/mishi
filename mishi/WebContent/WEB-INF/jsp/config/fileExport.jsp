...
function fillData(){
		if(config&&config.emailFlag=="1"){
			config.emailFlag=true;
		}else{
			config.emailFlag=false;
		}
		$("#csvAdd").val(config.csvAdd);
		//$("#emailFlag").val(config.emailFlag);
		liger.get("emailFlag").setValue(config.emailFlag);
}
function beforeSave(){
config.csvAdd=$("#csvAdd").val();
if(config.csvAdd==""){
	$.ligerDialog.warn("请输入有效的路径。","ERROR");
	return;
}
var flag=liger.get("emailFlag").getValue();
if(flag){
	config.emailFlag="1";
}else{
	config.emailFlag="0";
...
$("#csvAdd").val("D:\\dbBackup");
		//$("#emailFlag").val(config.emailFlag);
		liger.get("emailFlag").setValue(false);
}
...
<body>
	...
	<div class="first_div">
		<div class="title_div search_text" >数据导出设置</div>
		...
		<form>
            ...
            <tr>
                <td align="right" class="l-table-edit-td">数据导出地址:</td>
                <td align="left" class="l-table-edit-td">
                	<input type="text" id="csvAdd" name="csvAdd"/>
                </td>
				<td align="left" class="l-table-edit-td"></td>
            </tr>
            <tr>
                <td align="right" class="l-table-edit-td">是否发送邮件:</td>
                <td align="left" class="l-table-edit-td">
